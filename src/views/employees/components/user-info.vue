      ...
      <el-row class="inline-info">
        <el-col :span="12">
          <el-form-item label="员工头像">
            <!-- 放置上传图片 -->
            <ImageUpload ref="imgRef" :limit="1" :default-img="userInfo.staffPhoto" />
          </el-form-item>
        </el-col>
      </el-row>
      <!-- 保存个人信息 -->
...
export default {
  ...
  methods: {
    ...
    async getUserBaseInfo () {
      try {
        const ret = await getBaseInfo(this.userId)
        if (!ret.success) {
    ...
    async saveUser () {
      try {
        // 保存表单时，验证用户是否已经上传头像了
        const fileList = this.$refs.imgRef.fileList
        const isOk = fileList.every(item => item.status === 'success')
        if (!isOk) {
